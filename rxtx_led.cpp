...
void monitor_packets(const string& eth_if, const string& type, const string& led_shot, const string& msg, const string& thread_name) {
  pthread_setname_np(pthread_self(), thread_name.substr(0, 15).c_str()); // max 16 chars including null
...
int main(int argc, char* argv[]) {
  ...
  while (true) {
    if (fs::is_directory("/sys/class/net/" + eth_if)) {
      state = "active";
      running = true;
      thread tx_thread(monitor_packets, eth_if, "tx", TX_LED_SHOT, "Data transmitted", "tx_monitor");
      thread rx_thread(monitor_packets, eth_if, "rx", RX_LED_SHOT, "Data received", "rx_monitor");
      tx_thread.join();
      rx_thread.join();
      running = false;
    } else {
